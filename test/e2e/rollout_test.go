...
var _ = SIGDescribe("Rollout", func() {
	...
	ResumeRolloutCanary := func(name string) {
		...
		Eventually(func() bool {
			...
			return clone.Status.CanaryStatus.CurrentStepState == rolloutsv1alpha1.CanaryStepStatePaused
		}, 3*time.Minute, time.Second).Should(BeTrue())

		fmt.Println("resume currentStepState", util.DumpJSON(clone.Status))
	...
	KruiseDescribe("CloneSet rollout canary nginx", func() {
		...
		It("V1->V2: disable quickly rollback policy with traffic routing", func() {
			By("Creating Rollout...")
			rollout := &rolloutsv1alpha1.Rollout{}
			Expect(ReadYamlToObject("./test_data/rollout/rollout_canary_base.yaml", rollout)).ToNot(HaveOccurred())
			rollout.Spec.ObjectRef.WorkloadRef = &rolloutsv1alpha1.WorkloadRef{
