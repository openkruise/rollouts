...
var _ = SIGDescribe("Advanced Deployment", func() {
	...
	UpdateDeployment := func(deployment *apps.Deployment, version string) *apps.Deployment {
		...
		Expect(retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			strategy := unmarshal(clone.Annotations[rolloutsv1alpha1.AdvancedDeploymentStrategyAnnotation])
			strategy.Paused = true
			clone.Annotations[rolloutsv1alpha1.AdvancedDeploymentStrategyAnnotation] = marshal(strategy)
		...
		Eventually(func() bool {
			...
			By(fmt.Sprintf("%s %s", clone.Spec.Template.Spec.Containers[0].Image, clone.Spec.Template.Spec.Containers[0].Env[0].Value))
	...
	CheckReplicas := func(deployment *apps.Deployment, replicas, available, updated int32) {
		...
		Eventually(func() bool {
			...
			err := GetObject(deployment.Namespace, deployment.Name, clone)
			Expect(err).NotTo(HaveOccurred())
			fmt.Println(clone.Status.Replicas, clone.Status.AvailableReplicas, clone.Status.UpdatedReplicas)
	...
	UpdatePartitionWithCheck := func(deployment *apps.Deployment, desired intstr.IntOrString) {
		By(fmt.Sprintf("update deployment %v to desired: %v, strategy: %v, and check",
			client.ObjectKeyFromObject(deployment), deployment.Annotations[rolloutsv1alpha1.AdvancedDeploymentStrategyAnnotation], desired))
	...
	KruiseDescribe("Advanced Deployment Checker", func() {
		It("update with partition", func() {
			...
			Expect(ReadYamlToObject("./test_data/advanceddeployment/deployment.yaml", deployment)).ToNot(HaveOccurred())
		...
		It("update with MaxSurge=1, MaxUnavailable=0", func() {
			...
			deployment.Namespace = namespace
			Expect(ReadYamlToObject("./test_data/advanceddeployment/deployment.yaml", deployment)).ToNot(HaveOccurred())
			deployment.Annotations[rolloutsv1alpha1.AdvancedDeploymentStrategyAnnotation] =
				`{"type":"RollingUpdate","rollingUpdate":{"maxUnavailable":0,"maxSurge":1},"stableRevision":"1324324","desiredUpdatedReplicas":0}`
			CreateObject(deployment)
			CheckReplicas(deployment, 5, 5, 5)
		...
		It("update with MaxSurge=0, MaxUnavailable=1", func() {
			...
			Expect(ReadYamlToObject("./test_data/advanceddeployment/deployment.yaml", deployment)).ToNot(HaveOccurred())
			deployment.Annotations[rolloutsv1alpha1.AdvancedDeploymentStrategyAnnotation] =
				`{"type":"RollingUpdate","rollingUpdate":{"maxUnavailable":1,"maxSurge":0},"stableRevision":"1324324","desiredUpdatedReplicas":0}`
...
func unmarshal(strategyAnno string) *rolloutsv1alpha1.AdvancedDeploymentStrategy {
	strategy := &rolloutsv1alpha1.AdvancedDeploymentStrategy{}
	_ = json.Unmarshal([]byte(strategyAnno), strategy)
	return strategy
}

func marshal(strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) string {
