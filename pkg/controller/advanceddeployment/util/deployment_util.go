...
func SetNewReplicaSetAnnotations(deployment *apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy, newRS *apps.ReplicaSet, newRevision string, exists bool, revHistoryLimitInChars int) bool {
...
// FindActiveOrStable returns the oldest revision replica set or nil
func FindActiveOrStable(stableRS *apps.ReplicaSet, oldRSs []*apps.ReplicaSet) *apps.ReplicaSet {
	if stableRS == nil && len(oldRSs) == 0 {
		return nil
	}
	...
	sort.Sort(ReplicaSetsByRevision(oldRSs))
	return oldRSs[0]
}
...
func MaxUnavailable(deployment apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) int32 {
	if !IsRollingUpdate(strategy) || *(deployment.Spec.Replicas) == 0 {
		return int32(0)
	}
...
func MaxSurge(deployment apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) int32 {
	if !IsRollingUpdateStrategy(&strategy.DeploymentStrategy) {
		return int32(0)
	}
	// Error caught by validation
...
func GetProportion(rs *apps.ReplicaSet, d apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy, deploymentReplicasToAdd, deploymentReplicasAdded int32) int32 {
...
func getReplicaSetFraction(rs apps.ReplicaSet, d apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) int32 {
...
func IsRollingUpdate(strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) bool {
	return IsRollingUpdateStrategy(&strategy.DeploymentStrategy)
}

// IsRollingUpdateStrategy returns true if the strategy type is a rolling update.
func IsRollingUpdateStrategy(strategy *apps.DeploymentStrategy) bool {
	return strategy.Type == apps.RollingUpdateDeploymentStrategyType
}
...
func DeploymentRolloutBatchComplete(deployment *apps.Deployment, newRSDesiredReplicas int32) bool {
	return deployment.Status.ObservedGeneration >= deployment.Generation &&
		deployment.Status.Replicas == *(deployment.Spec.Replicas) &&
		deployment.Status.UpdatedReplicas >= newRSDesiredReplicas
}
...
func NewRSNewReplicas(deployment *apps.Deployment, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy, allRSs []*apps.ReplicaSet, newRS *apps.ReplicaSet) (int32, error) {
	// Check if we can scale up.
	maxSurge := MaxSurge(*deployment, strategy)
...
// IsUnderOurControl return true if this deployment should be controlled by advanced deployment controller.
func IsUnderOurControl(deployment *apps.Deployment) bool {
	return deployment.Annotations[util.BatchReleaseControlAnnotation] != "" &&
		strings.EqualFold(deployment.Annotations[rolloutsv1alpha1.DeploymentRolloutPolicyAnnotation], "batch")
}

// DisableNativeController make sure native controller would not fight with our advanced deployment controller.
func DisableNativeController(deployment *apps.Deployment) bool {
	return deployment.Spec.Paused && deployment.Spec.Strategy.Type == apps.RecreateDeploymentStrategyType
}
...
// WrapLessFucByImportanceNewer wrap Less func for sort.Slice, and allow to sort RSes by:
// - sorts a list of ReplicaSet by CreationTimeStamp in descending order;
// - make sure the stable RS is the last one.
func WrapLessFucByImportanceNewer(rs []*apps.ReplicaSet, strategy *rolloutsv1alpha1.AdvancedDeploymentStrategy) func(i, j int) bool {
	return func(i, j int) bool {
		if strategy.StableRevision == "" {
			if IsConsistentWithRevision(rs[i], strategy.StableRevision) {
				return false
			}
			if IsConsistentWithRevision(rs[j], strategy.StableRevision) {
				return true
			}
		}
		return ReplicaSetsByCreationTimestamp(rs).Less(j, i)
	}
}

// IsConsistentWithRevision return true iff the RS is matched with the revision.
func IsConsistentWithRevision(rs *apps.ReplicaSet, revision string) bool {
	return rs.Labels[apps.DefaultDeploymentUniqueLabelKey] != "" &&
		strings.HasSuffix(revision, rs.Labels[apps.DefaultDeploymentUniqueLabelKey])
}
