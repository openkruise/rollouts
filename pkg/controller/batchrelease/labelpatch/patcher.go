...
func (r *realPatcher) patchPodBatchLabel(pods []*corev1.Pod, ctx *batchcontext.BatchContext) error {
	plannedUpdatedReplicasForBatches := CalculatePlannedStepIncrements(ctx.Batches, int(ctx.Replicas), int(ctx.CurrentBatch))
	klog.InfoS("CalculatePlannedStepIncrements", "plannedUpdatedReplicasForBatches", plannedUpdatedReplicasForBatches)
	var updatedButUnpatchedPods []*corev1.Pod

	for _, pod := range pods {
		if !pod.DeletionTimestamp.IsZero() {
			klog.InfoS("Pod is being deleted, skip patching", "pod", klog.KObj(pod))
			continue
		}
		...
		// new/recreated pods should not be patched again
		if pod.Labels[v1beta1.RolloutIDLabel] != ctx.RolloutID {
			updatedButUnpatchedPods = append(updatedButUnpatchedPods, pod)
			klog.InfoS("Find a pod to add updatedButUnpatchedPods", "pod", klog.KObj(pod))
			continue
		}
	...
	klog.Infof("updatedButUnpatchedPods amount is %d", len(updatedButUnpatchedPods))
	// patch the pods
	for i := len(plannedUpdatedReplicasForBatches) - 1; i >= 0; i-- {
		for ; plannedUpdatedReplicasForBatches[i] > 0; plannedUpdatedReplicasForBatches[i]-- {
			if len(updatedButUnpatchedPods) == 0 {
				return fmt.Errorf("no pods to patch for %v, batch %d", r.logKey, i+1)
			}
			...
			klog.InfoS("Successfully patch Pod batchID", "batchID", i+1, "rollout", r.logKey, "pod", klog.KObj(pod))
			// update the counter
	...
	if len(updatedButUnpatchedPods) != 0 {
		...
		// return fmt.Errorf("still has %d pods to patch for %v", len(updatedButUnpatchedPods), r.logKey)
	}
	return nil
}

func CalculatePlannedStepIncrements(batches []v1beta1.ReleaseBatch, workloadReplicas, currentBatch int) (res []int) {
	...
	for i := currentBatch; i > 0; i-- {
		res[i] -= res[i-1]
		// webhook can ensure the steps replicas is non-decreasing
		if res[i] < 0 {
			panic("batch replicas increment is less than 0")
		}
	}
	return
}
