...
// NewUnifiedWorkloadRolloutControlPlane creates a new CloneSet rollout controller
...
func (c *UnifiedWorkloadRolloutControlPlane) VerifyWorkload() (bool, error) {
	...
	c.recorder.Event(c.planController, v1.EventTypeNormal, "VerifiedSuccessfully", "ReleasePlan and the CloneSet resource are verified")
	return true, nil
}
...
func (c *UnifiedWorkloadRolloutControlPlane) CheckOneBatchReady() (bool, error) {
	...
	klog.InfoS("checking the batch releasing progress", "BatchRelease", client.ObjectKeyFromObject(c.planController),
		"current-batch", c.planController.Status.CanaryStatus.CurrentBatch, "canary-ready-replicas", canaryReadyReplicas,
