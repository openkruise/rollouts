...
type UnifiedWorkloadController interface {
	...
	IgnoreNoNeedRollbackReplicas() (bool, error)
}
...
func (c *UnifiedWorkloadRolloutControlPlane) prepareBeforeRollback() (bool, *int32, error) {
	if c.planController.Annotations[util.RollbackInBatchAnnotation] == "" {
		return true, nil, nil
	}

	noNeedRollbackReplicas := int32(0)
	rolloutID := c.planController.Spec.ReleasePlan.RolloutID
	if rolloutID == "" {
	...
	klog.Infof("BatchRelease(%v) find %v replicas no need to rollback", client.ObjectKeyFromObject(c.planController), noNeedRollbackReplicas)
	return false, &noNeedRollbackReplicas, nil
}
...
func (c *UnifiedWorkloadRolloutControlPlane) UpgradeOneBatch() (bool, error) {
	...
	if c.newStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		rolloutID := c.planController.Spec.ReleasePlan.RolloutID
		noNeedRollbackReplicas = countNoNeedRollbackReplicas(pods, c.newStatus.UpdateRevision, rolloutID)
		c.newStatus.CanaryStatus.NoNeedRollbackReplicas = pointer.Int32(noNeedRollbackReplicas)
	}
	ignored, _ := c.IgnoreNoNeedRollbackReplicas()
	if ignored {
...
func (c *UnifiedWorkloadRolloutControlPlane) CheckOneBatchReady() (bool, error) {
	...
	if c.newStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		noNeedRollbackReplicas = *c.newStatus.CanaryStatus.NoNeedRollbackReplicas
	}
...
func (c *UnifiedWorkloadRolloutControlPlane) calculateCurrentCanary(totalSize int32) int32 {
	canaryGoal := int32(calculateNewBatchTarget(&c.planController.Spec.ReleasePlan, int(totalSize), int(c.planController.Status.CanaryStatus.CurrentBatch)))
...
func (c *UnifiedWorkloadRolloutControlPlane) patchPodBatchLabel(pods []*v1.Pod, plannedBatchCanaryReplicas, expectedBatchStableReplicas int32) (bool, error) {
	...
	pods, err := c.ListOwnedPods()
	if err != nil {
		klog.Errorf("Failed to list pods for BatchRelease %v", client.ObjectKeyFromObject(c.planController))
		return false, err
	}
	...
	if c.newStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		pods = filterPodsForRollback(pods, plannedBatchCanaryReplicas, expectedBatchStableReplicas, c.planController.Status.ObservedWorkloadReplicas, rolloutID, updateRevision)
	}
	return patchPodBatchLabel(c.client, pods, rolloutID, batchID, updateRevision, plannedBatchCanaryReplicas, client.ObjectKeyFromObject(c.planController))
}
