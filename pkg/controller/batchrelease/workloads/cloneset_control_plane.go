...
// prepareBeforeRollback makes sure that the updated pods have been patched rollback label
func (c *CloneSetRolloutController) prepareBeforeRollback() (bool, *int32, error) {
	if c.parentController.Annotations[util.RollbackInBatchAnnotation] != "true" {
		return true, nil, nil
	}

	noNeedRollbackReplicas := int32(0)
	rolloutID := c.releasePlan.RolloutID
	if rolloutID == "" {
...
func (c *CloneSetRolloutController) UpgradeOneBatch() (bool, error) {
	...
	var partitionedStableReplicas int
	var plannedBatchCanaryReplicas int32
	var expectedBatchCanaryReplicas int32
	var expectedBatchStableReplicas int32
	var expectedPartition intstr.IntOrString

	if c.releaseStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		noNeedRollbackReplicas = countNoNeedRollbackReplicas(pods, c.releaseStatus.UpdateRevision, c.releasePlan.RolloutID)
		c.releaseStatus.CanaryStatus.NoNeedRollbackReplicas = pointer.Int32(noNeedRollbackReplicas)
	}

	updatedReplicas = c.clone.Status.UpdatedReplicas
	replicas = c.releaseStatus.ObservedWorkloadReplicas
	currentBatch = c.releaseStatus.CanaryStatus.CurrentBatch
	...
	plannedBatchCanaryReplicas = c.calculateCurrentCanary(c.releaseStatus.ObservedWorkloadReplicas)
	...
	canaryIntOrStr := c.releasePlan.Batches[currentBatch].CanaryReplicas
	if canaryIntOrStr.Type == intstr.Int {
		...
		expectedPartition = ParseIntegerAsPercentageIfPossible(expectedBatchStableReplicas, c.releaseStatus.ObservedWorkloadReplicas, &canaryIntOrStr)
	}
...
func (c *CloneSetRolloutController) CheckOneBatchReady() (bool, error) {
	...
	var expectedBatchCanaryReplicas int32
	var expectedBatchStableReplicas int32
	var realNeedUpgradeCanaryReplicas int32
	var expectedPartition intstr.IntOrString

	if c.releaseStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		noNeedRollbackReplicas = *c.releaseStatus.CanaryStatus.NoNeedRollbackReplicas
	}
	...
	realNeedUpgradeCanaryReplicas = CalculateRealCanaryReplicasGoal(expectedBatchStableReplicas, replicas, &c.releasePlan.Batches[currentBatch].CanaryReplicas)
	...
	if !currentBatchIsReady {
		klog.InfoS("the batch is not ready yet", "BatchRelease", c.releasePlanKey, "current-batch", c.releaseStatus.CanaryStatus.CurrentBatch)
		return false, nil
	}
...
func (c *CloneSetRolloutController) patchPodBatchLabel(pods []*v1.Pod, plannedBatchCanaryReplicas, expectedBatchStableReplicas int32) (bool, error) {
	rolloutID := c.parentController.Spec.ReleasePlan.RolloutID
	if rolloutID == "" {
		return true, nil
	}
	...
	if c.releaseStatus.CanaryStatus.NoNeedRollbackReplicas != nil {
		pods = filterPodsForRollback(pods, plannedBatchCanaryReplicas, expectedBatchStableReplicas, c.parentController.Status.ObservedWorkloadReplicas, rolloutID, updateRevision)
	}
