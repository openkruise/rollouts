...
func (r *gatewayController) Initialize(ctx context.Context) (bool, error) {
	route := &gatewayv1alpha2.HTTPRoute{}
	err := r.Get(ctx, types.NamespacedName{Namespace: r.conf.RolloutNs, Name: *r.conf.TrafficConf.HTTPRouteName}, route)
	if err != nil {
		if errors.IsNotFound(err) {
			return false, fmt.Errorf("HTTPRoute(%s/%s) is Not Found", r.conf.RolloutNs, *r.conf.TrafficConf.HTTPRouteName)
		}
		return false, err
	}
	return true, nil
...
func (r *gatewayController) VerifyRoutes(ctx context.Context, desiredWeight int32) (bool, error) {
	...
	// End of rollout, restore gateway api related configuration
	if desiredWeight == 0 {
		// delete ns scenario
	...
	if errors.IsNotFound(err) {
		klog.Warningf("rollout(%s/%s) canary HTTPRoute not found", r.conf.RolloutNs, r.conf.RolloutName)
		return false, nil
	}
	// desired rule
	desiredRule := r.buildDesiredHTTPRoute(httpRoute.Spec.Rules, desiredWeight)
	if !reflect.DeepEqual(httpRoute.Spec.Rules, desiredRule) {
		klog.Infof("rollout(%s/%s) verify trafficRouting(weight: %d) invalid and update", r.conf.RolloutNs, r.conf.RolloutName, desiredWeight)
		return false, nil
	}
