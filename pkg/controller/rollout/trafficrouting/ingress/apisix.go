...
func NewApisixIngressTrafficRouting(client client.Client, conf Config) (trafficrouting.Controller, error) {
...
func (r *apisixIngressController) EnsureRoutes(ctx context.Context, weight *int32, matches []rolloutv1alpha1.HttpRouteMatch) (bool, error) {
	if *weight == 0 {
...
func (r *apisixIngressController) Finalise(ctx context.Context) (bool, error) {
	apisixRoute := &a6v2.ApisixRoute{}
	err := r.Get(ctx, types.NamespacedName{Namespace: r.conf.RolloutNs, Name: r.conf.TrafficConf.Name}, apisixRoute)
	if err != nil {
		klog.Errorf("rollout(%s/%s) get apisix route(%s) failed: %s", r.conf.RolloutNs, r.conf.RolloutName, r.conf.TrafficConf.Name, err.Error())
	...
	for index, thr := range targetHTTPRoutes {
		...
		if !canaryBackendExists {
			return false, fmt.Errorf("rollout(%s/%s) get apisix route canary backend(%s) failed", r.conf.RolloutNs, r.conf.RolloutName, r.conf.CanaryService)
		}
