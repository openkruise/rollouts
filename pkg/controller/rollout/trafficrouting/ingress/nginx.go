...
func NewNginxIngressTrafficRouting(client client.Client, conf Config) (trafficrouting.Controller, error) {
	r := &nginxIngressController{
		Client:            client,
		conf:              conf,
...
func (r *nginxIngressController) Initialize(ctx context.Context) error {
...
func (r *nginxIngressController) EnsureRoutes(ctx context.Context, weight *int32, matches []rolloutv1alpha1.HttpRouteMatch) (bool, error) {
	canaryIngress := &netv1.Ingress{}
...
func (r *nginxIngressController) Finalise(ctx context.Context) (bool, error) {
	canaryIngress := &netv1.Ingress{}
	err := r.Get(ctx, types.NamespacedName{Namespace: r.conf.RolloutNs, Name: r.canaryIngressName}, canaryIngress)
	if err != nil && !errors.IsNotFound(err) {
		klog.Errorf("rollout(%s/%s) get canary ingress(%s) failed: %s", r.conf.RolloutNs, r.conf.RolloutName, r.canaryIngressName, err.Error())
...
func (r *nginxIngressController) buildCanaryIngress(stableIngress *netv1.Ingress) *netv1.Ingress {
...
func (r *nginxIngressController) executeLuaForCanary(annotations map[string]string, weight *int32, matches []rolloutv1alpha1.HttpRouteMatch) (map[string]string, error) {
	if weight == nil {
...
func (r *nginxIngressController) getTrafficRoutingIngressLuaScript(iType string) (string, error) {
	if iType == "" {
		iType = "nginx"
	}
