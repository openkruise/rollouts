...
type Config struct {
	...
	TrafficConf   []rolloutv1alpha1.NetworkTrafficRouting
	OwnerRef      metav1.OwnerReference
}

func NewCustomController(client client.Client, conf Config, lua string) (network.NetworkProvider, error) {
	r := &customController{
		Client:     client,
		conf:       conf,
		luaManager: &luamanager.LuaManager{},
...
func (r *customController) EnsureRoutes(ctx context.Context, weight *int32, matches []rolloutv1alpha1.HttpRouteMatch) (bool, error) {
	var err error
	var done = true
	for _, ref := range r.conf.TrafficConf {
		obj := &unstructured.Unstructured{}
		...
		nSpec, err := r.executeLuaForCanary(oSpec, weight, matches)
		if err != nil {
			return false, err
		}
...
func (r *customController) executeLuaForCanary(spec interface{}, weight *int32, matches []rolloutv1alpha1.HttpRouteMatch) (interface{}, error) {
	if weight == nil {
	...
	l, err := r.luaManager.RunLuaScript(u, r.luaScript)
	if err != nil {
		return nil, err
	}
	returnValue := l.Get(-1)
